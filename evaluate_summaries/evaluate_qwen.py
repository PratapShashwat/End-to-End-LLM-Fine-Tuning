...
def main():
    ...
    # 1. Load the input .out text file
    with open(args.input, 'r', encoding='utf-8') as f:
        content = f.read()

    # --- PARSE CUSTOM LOG FORMAT ---
    # We use regex to extract the ID, Generated Summary, and Reference.
    # re.DOTALL allows the .* to match across multiple lines.
    pattern = r"=========== Document (\d+) ===========\s*Generated Summary:\s*(.*?)\s*REFERENCE:\s*(.*?)\s*ROUGE SCORES:"
    ...
    for match in matches:
        data.append({
            "id": match[0],
            "generated": match[1].strip(),
            "reference": match[2].strip()
        })
        
    print(f"Successfully extracted {len(data)} documents from the log file.")
